...
namespace WebAppService.Areas.Admin.Controllers
{
    [Area("Admin")]
    [Authorize]
    public class PosterHomeController : Controller
    {
        public IActionResult PosterHome()
        {
            return View();
        }
        ...
        public IActionResult LoadTable()
        {
            try
            {
                var lstData = db.BrowerHomePages.Where(c => c.SoThuTu != null).AsNoTracking().Select(c => new
                {
                    c.Id,
                    c.SeoUrl,
                    c.SeoTittile,
                    ...
                    c.PhanLoai,
                    c.IsBanerHome,
                }).OrderByDescending(c => c.IsBanerHome).ToList();
